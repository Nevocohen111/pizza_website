...
export default function MyOrders() {
    ...
    const handleDeleteById = (id) => {
        ...
        deleteByIdRef = setTimeout(() => {
            ...
            fetch('http://localhost:8080/user/delete', requestOptions)
            window.location.reload();
        }, 500);
        return () => {
            clearTimeout(deleteByIdRef.current);
        }
            ...
            {userData?.length > 0 ?
                <div>
                    {userData.map((item, index) => (
                        <div className="container ml-48" ref={cardRef} id={item.id} style={{ position: "relative", top: '-5rem' }} key={index}>
                            <div className="flex flex-col items-center justify-center mt-10"  >
                                <div className="ui padded text container segment mt-10" style={{ opacity: "0.8", backgroundColor: '#E97451' }}>
                                    ...
                                    <img src="https://img.pizza/100/100" alt='Changing random Pizza api ' style={{ borderRadius: '50%' }} className="mb-8 ml-72" />
